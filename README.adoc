...
. Navigate to the Dev Portal of the 3scale Tenant and click the link:
. Complete the registration form using a valid email address.  Example as follows:
+
image::doc/images/dev_portal_registration_form.png[]

. An account validation email will have been sent.  Click the link in the email to activate the account
+
